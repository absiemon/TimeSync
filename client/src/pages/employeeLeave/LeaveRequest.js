...
function LeaveRequest() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if(user && user?.role==='admin'){
        result = await axios.get(`http://localhost:8000/api/leave/get?type=request&name=${filterByName}`);
      }
      else{
        result = await axios.get(`http://localhost:8000/api/leave/get-leave/${user?.id}?type=request&name=${filterByName}`);
      }
      setLoading(false)
  ...
  const handleStatus = (status, id, emp_id) => {
    let newStatus;
    if(status === "accept"){
        newStatus = "accepted"
    }
      ...
      if (result.isConfirmed) {
        Swal.fire(
          ...
          'success'
            ...
            const result = await axios.post('http://localhost:8000/api/leave/update-leave-request',
            {id, status:newStatus, emp_id}
            );
            setFetchAgain(!fetchAgain)
      }
    })
      ...
      renderCell: (params) => (
        <>
          {user.role==="admin" && <Button
            aria-label="accept"
            className='punch bg-green-600 text-white'
            onClick={() => handleStatus("accept", params.row.id, params.row.emp_id)}
          >
            accept
          </Button>}
          {user.role==="admin" && <Button
            aria-label="reject"
            className='punch bg-red-600 text-white'
            style={{ margin: "0px 10px" }}
            onClick={() => handleStatus("reject", params.row.id, params.row.emp_id)}
          >
            reject
          </Button>}
          <Link to={`/leave/create/${params.row.id}`}>
            <Button aria-label="edit" className='punch bg-white'>
              <EditOutlined />
            </Button>
          </Link>
        </>
