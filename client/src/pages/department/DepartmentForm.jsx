...
function DepartmentForm() {
    ...
    useEffect(() => {
        if (id) {
            const fetchData = async () => {
                try {
                    const res = await axios.get(`http://localhost:8000/api/department/get/${id}`);
    ...
    const handleFinish = async (values) => {
        ...
        try {
            if (id) {
                console.log(values)
                await axios.put(`http://localhost:8000/api/department/update/${id}`, values);
                setButtonLoading(false)
                navigate("/employee/department")
            } else {
                await axios.post('http://localhost:8000/api/department/create', values);
                setButtonLoading(false)
                navigate("/employee/department")
            }
                ...
                <Row gutter={[24, 0]}>
                    <Col xs="24" xl={24}>
                        <Card
                            ...
                            title={id ? "Department Form Edit" : "Department Form"}
                            extra={
                                <>
