...
function Designation() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const result = await axios.get(`http://localhost:8000/api/designation/get?name=` + filterByName);
  ...
  const handleDelete = (sr_no) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios.delete(`http://localhost:8000/api/designation/delete/${sr_no}`);
        setDesignations(designations.filter((item) => item.id !== sr_no));
      }
    })
  };
  ...
  const actionColumn = [
    {
      ...
      renderCell: (params) => (
        <>
          <Link to={`/employee/designation/create/${params.row.id}`}>
    ...
    <>
      <header className='p-8'>
        <div className='flex justify-between pb-6'>
          <h2 className='text-xl text-white'>Designation</h2>
          <Button type="primary" htmlType="submit" style={{ margin: "0px 10px", backgroundColor: "limegreen" }} className='punch' onClick={()=> navigate('/employee/designation/create')}>
            Add Designation
          </Button>
        </div>
